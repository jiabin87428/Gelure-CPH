...
//        NSLog(@"%@",[request.URL isEqual:[request mainDocumentURL]]?@"YES":@"NO");
        BOOL isTopLevelNavigation = isDevToolsRefresh || [request.URL isEqual:[request mainDocumentURL]];
        NSLog(@"%@",[[NSString stringWithFormat:@"%@",webView.request] hasSuffix:@"info.html"]?@"YES":@"NO");
        BOOL haveInfo = NO;
        NSRange range = [[NSString stringWithFormat:@"%@",webView.request] rangeOfString:@"info.html"];//判断字符串是否包含
        if (range.length >0){//包含
            haveInfo = YES;
        }else{//不包含
            haveInfo = NO;
        }
//        if (isTopLevelNavigation || haveInfo) {
